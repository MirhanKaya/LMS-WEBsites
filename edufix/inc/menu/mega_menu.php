...
if ( ! class_exists( 'Edufix_Mega_Menu' ) ) {
	class Edufix_Mega_Menu {
		var $_options;
		...
		public static function options() {
			return array(
				'edufix_wide_menu_enabled' => array(
					'type' 		=> 'select',
					'label' 	=> esc_html__( 'Enable Wide Menu', 'edufix' ),
					'default' => 0,
					'options' => array( 
						1 => esc_html__( 'Yes', 'edufix' ),
						0 => esc_html__( 'No', 'edufix' )
					),
					'size' => 'thin',
					'class' => 'edufix-show-only-depth-0',
				)				
			);
		}
			...
			if (!$this->_custom_options_is_parsed && isset($_REQUEST['nav-menu-data'])) {
				...
				if ($data_object) {
					foreach ($data_object as $key => $value) {
						if (strpos($value->name, 'menu-item-edufix_wide_menu_enabled') === 0) {
