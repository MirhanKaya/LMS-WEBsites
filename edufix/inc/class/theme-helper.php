 ...
 *
 *
 * @class        Edufix_Theme_Helper
 * @version      1.0
 * @category Class
 * @author       Xcodexe
 */

if (!class_exists('Edufix_Theme_Helper')) {
	Class Edufix_Theme_Helper {
		...
		function __construct() {
			...
			if (!function_exists('edufix_option')) {
				function edufix_option($option = '', $default = null) {
					$options = get_option('edufix_framework'); // Attention: Set your unique id of the framework

					return (isset($options[$option])) ? $options[$option] : $default;
				}
			}

			add_action('edufix_after_body', array($this, 'edufix_preloader_markup'), 1);
		}
		...
		public static function options_compare( $name, $check_key = false, $check_value = false ) {
            ...
            if ($check_key) {
                ...
                $var = $meta[$name];
                $option = !empty($var) ? $meta[$name] : edufix_option($name);
            }
		 ...
		 */
		//Add breadcrumbs
		static function edufix_breadcrumb() {
			global $post;
			$edufix_opt = get_option('edufix_opt');
			...
			if (!is_home()) {
				...
				if (is_category() || is_single()) {
					...
					printf(esc_html__('Archive for: %s', 'edufix'), '<span>' . get_the_date() . '</span>');
				} elseif (is_month()) {
					printf(esc_html__('Archive for: %s', 'edufix'), '<span>' . get_the_date(_x('F Y', 'monthly archives date format', 'edufix')) . '</span>');
				} elseif (is_year()) {
					printf(esc_html__('Archive for: %s', 'edufix'), '<span>' . get_the_date(_x('Y', 'yearly archives date format', 'edufix')) . '</span>');
				} elseif (is_author()) {
					echo "<span>" . esc_html__('Archive for', 'edufix');
					echo '</span>';
				} elseif (isset($_GET['paged']) && !empty($_GET['paged'])) {
					echo "<span>" . esc_html__('Blog Archives', 'edufix');
					echo '</span>';
				} elseif (is_search()) {
					echo "<span>" . esc_html__('Search Results', 'edufix');
					echo '</span>';
				}
				echo '</div>';
			} else {
				...
				echo esc_html__('Home', 'edufix');
				echo '</a>' . $brseparator;
				if (isset($edufix_opt['blog_header_text']) && $edufix_opt['blog_header_text'] != "") {
					echo esc_html($edufix_opt['blog_header_text']);
				} else {
					echo esc_html__('Blog', 'edufix');
				}
				echo '</div>';
			}
		}
		 ...
		 * @since edufix 1.0
		 */
		static function edufix_content_nav($html_id) {
			global $wp_query;
			$html_id = esc_attr($html_id);
			if ($wp_query->max_num_pages > 1) : ?>
                <nav id="<?php echo esc_attr($html_id); ?>" class="navigation" role="navigation">
                    <h3 class="assistive-text"><?php esc_html_e('Post navigation', 'edufix'); ?></h3>
                    <div class="nav-previous"><?php next_posts_link(wp_kses(__('<span class="meta-nav">&larr;</span> Older posts', 'edufix'), array('span' => array('class' => array())))); ?></div>
                    <div class="nav-next"><?php previous_posts_link(wp_kses(__('Newer posts <span class="meta-nav">&rarr;</span>', 'edufix'), array('span' => array('class' => array())))); ?></div>
                </nav>
			<?php endif;
		}
		...
		static function edufix_post_pagination($nav_query = false) {
		 ...
		 * simply create your own edufix_comment(), and that function will be used instead.
		 *
		 * Used as a callback by wp_list_comments() for displaying the comments.
		 *
		 * @since edufix 1.0
		 */
		static function edufix_comment($comment, $args, $depth) {
			...
			switch ($comment->comment_type) :
				...
				case 'trackback' :
                    ...
                    <p><?php esc_html_e('Pingback:', 'edufix'); ?><?php comment_author_link(); ?><?php edit_comment_link(esc_html__('(Edit)', 'edufix'), '<span class="edit-link">', '</span>'); ?></p>
					<?php
					break;
				default :
                        ...
                        <div class="comment-info">
                            <header class="comment-meta comment-author vcard">
								...
								printf('<cite><b class="fn">%1$s</b> %2$s</cite>',
									...
									($comment->user_id === $post->post_author) ? '<span>' . esc_html__('Post author', 'edufix') . '</span>' : ''
								);
								printf('<time datetime="%1$s">%2$s</time>',
									get_comment_time('c'),
									/* translators: 1: date, 2: time */
									sprintf(esc_html__('%1$s at %2$s', 'edufix'), get_comment_date(), get_comment_time())
								);
								?>
                                <div class="reply">
									<?php comment_reply_link(array_merge($args, array(
										'reply_text' => esc_html__('Reply', 'edufix'),
										'after' => '',
										'depth' => $depth,
							...
							<?php if ('0' == $comment->comment_approved) : ?>
                                <p class="comment-awaiting-moderation"><?php esc_html_e('Your comment is awaiting moderation.', 'edufix'); ?></p>
							<?php endif; ?>
                            <section class="comment-content comment">
								<?php comment_text(); ?>
								<?php edit_comment_link(esc_html__('Edit', 'edufix'), '<p class="edit-link">', '</p>'); ?>
		 ...
		 *
		 * Create your own edufix_entry_meta() to override in a child theme.
		 *
		 * @since edufix 1.0
		 */
		static function edufix_entry_meta() {
			...
			if (comments_open()) {
				if ($num_comments == 0) {
					$comments = esc_html__('0 comments', 'edufix');
				} elseif ($num_comments > 1) {
					$comments = $num_comments . esc_html__(' comments', 'edufix');
				} else {
					$comments = esc_html__('1 comment', 'edufix');
				}
				$write_comments = '<a href="' . get_comments_link() . '">' . $comments . '</a>';
			}
			$utility_text = null;
			if ((post_password_required() || !comments_open()) && ($tag_list != false && isset($tag_list))) {
				$utility_text = esc_html__('Tags: %2$s', 'edufix');
			} elseif ($tag_list != false && isset($tag_list) && $num_comments != 0) {
				$utility_text = esc_html__('%1$s / Tags: %2$s', 'edufix');
			} elseif (($num_comments == 0 || !isset($num_comments)) && $tag_list == true) {
				$utility_text = esc_html__('Tags: %2$s', 'edufix');
			} else {
				$utility_text = esc_html__('%1$s', 'edufix');
			}
			printf($utility_text, $write_comments, $tag_list);
		}

		static function edufix_entry_meta_small() {
			// Translators: used between list items, there is a space after the comma.
			$categories_list = get_the_category_list(', ');
			$author = sprintf('<span class="author vcard"><a class="url fn n" href="%1$s" title="%2$s" rel="author">%3$s</a></span>',
				esc_url(get_author_posts_url(get_the_author_meta('ID'))),
				esc_attr(sprintf(wp_kses(__('View all posts by %s', 'edufix'), array('a' => array())), get_the_author())),
				get_the_author()
			);
			$utility_text = esc_html__('By %1$s', 'edufix');
			printf($utility_text, $author);
		}


		static function edufix_posted_author_avatar() {
			global $post;
			printf(
		...
		static function edufix_posted_on() {
		...
		static function edufix_posted_by() {
			$byline = sprintf(
			/* translators: %s: post author. */
				esc_html_x('%s', 'post author', 'edufix'),
		...
		static function edufix_entry_cat() {
			// Hide category and tag text for pages.
			if ('post' === get_post_type()) {
				/* translators: used between list items, there is a space after the comma */
				$categories_list = get_the_category_list(esc_html__(' ', 'edufix'));
				if ($categories_list) {
					/* translators: 1: list of categories. */
					printf(esc_html__(' %1$s', 'edufix'), $categories_list); // WPCS: XSS OK.
				}
		...
		static function edufix_posted_tag() {
			// Hide category and tag text for pages.
			if ('post' === get_post_type()) {

				/* translators: used between list items, there is a space after the comma */
				$tags_list = get_the_tag_list('', esc_html_x(' ', 'list item separator', 'edufix'));
				if ($tags_list) {
					/* translators: 1: list of tags. */
					printf('<div class="tagcloud">' . esc_html__('%1$s', 'edufix') . '</div>', $tags_list); // WPCS: XSS OK.
				}
			}
		}
		 ...
		 * Trim text
		 */
		static function edufix_substring($string, $limit, $afterlimit = '[...]') {
			if (empty($string)) {
				return $string;
			}
		 ...
		 * @since edufix 1.0
		 */
		public function edufix_get_footer_column($footer_columns) {
		...
		public static function render_sidebars( $args = 'blog' ) {
			...
			$layout =
            edufix_option($args . '_sidebar_layout') ?
            edufix_option($args . '_sidebar_layout') : 'right';
			$sidebar = 'sidebar_main-sidebar';
			$sidebar_width = edufix_option($args . '_sidebar_def_width', '8');
			$sidebar_gap = edufix_option($args . '_sidebar_gap', '45');
		...
		private function post_author($author) {
			...
			return '<span class="post_author">' . esc_html__("by", "edufix") . ' <a href="' . esc_url(get_author_posts_url(get_the_author_meta('ID'))) . '">' . esc_html(get_the_author_meta('display_name')) . '</a></span>';
		}

		/**
		 * Display Post Thumbnail.
		 */
		static function edufix_post_thumbnail() {
		...
		static function render_author_info() {
			...
			$name_html = !empty($user_first) || !empty($user_last) ? '<h6 class="author">'.esc_html__('Author', 'edufix').'</h6>'.'<h5 class="author-info_name">' . $user_first . ' ' . $user_last . '</h5>' : '';
		...
		public function edufix_preloader_markup() {
			$preloader = edufix_option('preloader');
			$preloader_opt = edufix_option('edufix_preloader');
		...
		static function edufix_entry_comments($post_id, $no_comments = 'No Comment') {

			$comments_number = get_comments_number($post_id);
			if ($comments_number == 0) {
				$comment_text = $no_comments;
			} elseif ($comments_number == 1) {
				$comment_text = esc_html__('1 Comment', 'edufix');
			} elseif ($comments_number > 1) {
				$comment_text = $comments_number . esc_html__(' Comments', 'edufix');
			}
		...
		public static function branding_logo() {
			...
			$logo_main        = edufix_option( 'main_logo' );
			$sticky           = edufix_option( 'sticky_logo' );
			$retina_logo      = edufix_option( 'retina_logo' );
			$retina_sticky    = edufix_option( 'retina_logo_sticky' );
			$is_sticky_header = edufix_option( 'header_sticky' );
            ...
            <a href="<?php echo esc_url( home_url( '/' ) ); ?>" rel="home">
				...
				<?php } else { ?>
                    <h3><?php echo esc_html__('Edufix', 'edufix'); ?></h3>
				<?php } ?>
            </a>
            <?php
		}
		...
		static function related_post() { ?>
		    <div class="related-post-wrapper">
				<h2 class="related-title"><?php echo esc_html( edufix_option('related_title')); ?></h2>
                 <div class="row">
                     <?php
                      global $post;
                      $post_id = get_the_ID();
                                ...
                                <div class="col-md-6">
                                   <div class="related-post">
                                       ...
                                       <div class="blog-content">
                                           <ul class="post-meta">
                                               <li><?php self::edufix_entry_cat(); ?></li>
                                           </ul><!-- .entry-meta -->
                                           ...
                                           <h3 class="post-title">
											...
											<p>
												<?php echo self::edufix_substring(get_the_content(), 20, '...'); ?>
											</p>

											<ul class="post-footer-meta">
                                                <li class="author-simple"><i class="far fa-user"></i><?php echo Edufix_Theme_Helper::edufix_posted_by(); ?></li>
												<li><i class="fas fa-calendar-alt"></i><?php self::edufix_posted_on(); ?></li>												
											</ul><!-- .entry-meta -->
	   ...
	   static function edufix_post_nav() {
				...
				if ( !empty( $pre_post ) ):
					...
					<a href="<?php echo get_the_permalink( $pre_post->ID ); ?>" class="single-post-nav">
						...
						<div class="post-nav-wrapper">
							<p class="post-nav-title"><?php esc_html_e( 'Older Post', 'edufix' ) ?></p>
			...
			if ( !empty( $next_post ) ):
				...
				<a href="<?php echo get_the_permalink( $next_post->ID ); ?>" class="single-post-nav">				
					...
					<div class="post-nav-wrapper">
						<p class="post-nav-title"><?php esc_html_e( 'Next Post', 'edufix' ) ?></p>
		...
		static function edufix_portfolio_post_nav() {
			// Don't print empty markup if there's nowhere to navigate.
			$pre_post = $next_post = '';
			$next_post	 = get_next_post();
			$pre_post	 = get_previous_post();
		 ...
		 * @return array
		 */
		static function edufix_get_title_tag($first_empty = false, $additional_elements = array())	{
			$title_tag = array();
